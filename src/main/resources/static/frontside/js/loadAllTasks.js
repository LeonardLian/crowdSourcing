    ...
    $.get("http://127.0.0.1:8080/checkAllTasks",function (data) {
        ...
        if(data.length==0){
            ...
            for(var x in arr){
                ...
                $('#taskList').prepend('<li> <a href="javascript:taskdetailsClick(username,taskname);"> <img class="am-img-thumbnail am-img-bdrs" ' +
                    'src="data:image/jpeg;base64,'+result+'" alt=""/> <div class="gallery-title">'+tasktag+'</div> <div class="gallery-desc">人数：'
                    +numOfPart+'/'+numOfNeeded+'</div> <div class="gallery-desc">截止：'+deadline+'</div> </a> </li>');
